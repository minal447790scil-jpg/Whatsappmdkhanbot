...
async function getRedGifsVideos() {
    try {
        ...
        const response = await axios.get(
            'https://api.redgifs.com/v2/gifs/trending?count=50',
            {
                timeout: 15000,
                headers: {
                    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/120.0.0.0 Safari/537.36',
...
async function getRedGifsSearchVideos() {
    try {
        ...
        const response = await axios.get(
            `https://api.redgifs.com/v2/gifs/search?search_text=${encodeURIComponent(search)}&count=30`,
            {
                timeout: 15000,
                headers: {
                    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/120.0.0.0 Safari/537.36',
...
async function getAdultVideos() {
    ...
    let videos = await getRedGifsVideos();
    
    if (!videos || videos.length < 5) {
        console.log('🔄 Trying search method...');
        const searchVideos = await getRedGifsSearchVideos();
        if (searchVideos && searchVideos.length) {
            videos = videos.concat(searchVideos);
        }
    }
