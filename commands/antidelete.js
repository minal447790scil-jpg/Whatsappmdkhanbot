...
const MESSAGE_DB_PATH = path.join(__dirname, '../data/messages.json');
...
const cleanTempFolderIfLarge = () => {
    try {
        if (getFolderSizeInMB(TEMP_MEDIA_DIR) > 500) {
...
function loadMessages() {
    try {
        if (!fs.existsSync(MESSAGE_DB_PATH)) {
            fs.writeFileSync(MESSAGE_DB_PATH, JSON.stringify({}, null, 2));
        }
        return JSON.parse(fs.readFileSync(MESSAGE_DB_PATH, "utf8"));
    } catch (err) {
...
function saveMessages(data) {
    try {
        fs.writeFileSync(MESSAGE_DB_PATH, JSON.stringify(data, null, 2));
    } catch (err) {
...
function cleanupMessages() {
    ...
    for (const id in db) {
        if (now - db[id].timestamp > 30 * 24 * 60 * 60 * 1000) {

            if (db[id].mediaPath && fs.existsSync(db[id].mediaPath)) {
                try {
                    fs.unlinkSync(db[id].mediaPath);
                } catch {}
            }
    ...
    if (changed) saveMessages(db);
}   // 👈 Yeh closing bracket missing hai
...
async function handleMessageRevocation(sock, revocationMessage) {
    try {
        ...
        const db = loadMessages();
const original = db[messageId];
        if (!original) return;

        const sender = original.sender;
        const senderName = sender.split('@')[0];
        
        const isStatus = revocationMessage.key.remoteJid === 'status@broadcast';
...
`;
    ...
    report = `╭━━━〔 ${toBold("ANTI-DELETE REPORT")} 〕━━━┈⊷
┃ 👤 ${toBold("Sender:")} @${senderName}
┃ 🗑️ ${toBold("Deleted By:")} @${deletedBy.split('@')[0]}
┃ 🕒 ${toBold("Time:")} ${new Date().toLocaleTimeString("en-PK", {
    timeZone: "Asia/Karachi",
    hour: "2-digit",
    minute: "2-digit",
    second: "2-digit",
...
`;
    ...
    report += isStatus
        ...
        if (original.mediaType && fs.existsSync(original.mediaPath)) {
            const mediaOptions = {
    caption: isStatus
        ? `🗑️ *Deleted Status From:* @${senderName}`
        : `*Deleted ${original.mediaType}* from @${senderName}`,
    mentions: [sender]
};
